...
from test_model.test_demo import draw_point, draw_match_bf
...
def drawMatchPoints(image_name, img1, pts1, img2, pts2, desc1, desc2, cvBFSpp, win, display_scale=1):
    ...
    cv2.putText(stereo_img, f"Superpoint matches: {len(matches)}", (cols // 2 - 60, 20),
...
class SuperPointFrontend(object):
    ...
    def __init__(self, weights_path, nms_dist, conf_thresh, nn_thresh,topk=0,
                 cuda=False):
...
class VideoStreamer(object):
    ...
    def read_image(self, impath, img_size):
        ...
        if grayim is None or bgrim is None:
    ...
    def next_frame(self):
        ...
        if self.camera:
            ...
            # image_file = "/data1/Parker/BASE/TRAIN/shujuji0310/小广场/recordgczaochen100/imsee_data.bag.imgs.L/1614044732049807_L.png"
            input_image,bgr_image = self.read_image(image_file, self.sizer)
        # Increment internal counter.
        self.i = self.i + 1
        input_image = input_image.astype('float32')
        image_name = os.path.basename(image_file)
        # print('image_name', image_name)
        return (input_image, image_name, True,bgr_image)



class XFeatFrontend(object):
    def __init__(self,img1,img2,topk=1000):
        self.img1 = img1
        self.img2 = img2
        self.topk = topk
    ...
    def get_point_desc(self):
        xfeat = XFeat()
        matches, pts1, pts2 = xfeat.match_xfeat_BFMatcher(self.img1, self.img2, self.topk)
        img_point = draw_point(pts1, pts2, self.img1, self.img2)
        img_match = draw_match_bf(self.img1, self.img2, matches, pts1, pts2)
        cv2.putText(img_point, f"Xfeat KeyPoints1: {len(pts1)}", (10, 20),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        cv2.putText(img_point, f"KeyPoints2: {len(pts2)}", (10 + int(self.img1.shape[1]), 20),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        cv2.putText(img_match, f"Xfeat matches: {len(matches)}", (self.img1.shape[1]-60, 20),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
        return img_point,img_match
...
if __name__ == '__main__':
    ...
    parser.add_argument('--topk', type=int, default=500,
                        help='Get the first n points (default: 800).')
    ...
    # f = open(f'./datasets/test_data/R101_102_test_data/{opt.log_name}.txt', 'w')
    # print(f"{opt.log_name}正在记录点...")
    ...
    while True:
        ...
        # Get a new image.
        try:
            img, image_name, status,bgr_img = vs.next_frame()
        except:
            break
        if status is False:
            break
        try:
            img2, image_name2, status2,bgr_img2 = vs2.next_frame()
        except:
            break
        if status2 is False:
            break
        ...
        cv2.putText(out1, f"Superpoint KeyPoints1: {len(pts.T)}", (10, 20),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        ...
        cv2.putText(out2, f"KeyPoints2: {len(pts2.T)}", (10, 20),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        ...
        if not opt.no_display:
            show = np.hstack([out1, out2])
            descimg = drawMatchPoints(image_name, img, pts, img2, pts2, desc, desc2, cvBFSpp, win3, opt.display_scale)
            # cv2.imshow(win1, show)
            # cv2.imshow(win3,descimg)
            # detectAndCompute(image_name, img, img2, orb, cvBFORB, opt.display_scale)
            xfeat_point,xfeat_match= XFeatFrontend(bgr_img,bgr_img2,opt.topk).get_point_desc()
            # cv2.imshow("xfeat_point",xfeat_point)
            # cv2.imshow("xfeat_match",xfeat_match)
            res_point = np.vstack((show,xfeat_point))
            res_desc = np.vstack((descimg,xfeat_match))
            cv2.imshow("points",res_point)
            cv2.imshow("matches",res_desc)
            if key == 27:
        ...
        # Optionally write images to disk.
            if opt.write:
                # print(image_name)
                try:
                    out_file1 = os.path.join(opt.write_dir, "t"+image_name)
                    print('Writing image to %s' % out_file1)
                    cv2.imwrite(out_file1, show)
                    out_file2 = os.path.join(opt.write_dir, "d"+image_name)
                    print('Writing image to %s' % out_file2)
                    cv2.imwrite(out_file2, descimg)
...
"""
训练后的命令测试：
python3 demo_superpoint_orb.py --log_name="104_204_best_小广场高光" --input1="/data1/Parker/BASE/TRAIN/shujuji0310/小广场/recordgcgaoguang500/imsee_data.bag.imgs.L/" --input2="/data1/Parker/BASE/TRAIN/shujuji0310/小广场/recordgcgaoguang500/imsee_data.bag.imgs.R/" --W=640 --H=400 --cuda --display_scale=1 --weights_path="test_weights/R1.0.4_weight/R104_best_weight.pth" --write --write_dir=img_out/R103_104_test/104_204_best_小广场高光/

/media/xin/data1/shujuji0310/小广场/高光/recordgcgaoguang500/imsee_data.bag.imgs.L
/media/xin/data1/shujuji0310/花坛/下午/recordhtxiawu500/imsee_data.bag.imgs.L
